 ...
 */
	 ...
	 * 
	 * @param double x
	 * @return deg point for x after being converted to meters
	 */
	private double latrad2latmet(double x) {
		return Math.sin(Math.toRadians(x))*6371000;
		}
	/**
	 * 
	 * @param double x
	 * @return deg point for y after being converted to meters
	 */
	private double lonrad2lonmet(double x) {
		return Math.sin(Math.toRadians(x))*0.847091*6371000;
		}
	/**
	 * change coordinates of the point from deg --> meters ,x&y
	 */
	public void point2meters() {
		this._x=latrad2latmet(this._x);
		this._y=lonrad2lonmet(this._y);
		}
}
